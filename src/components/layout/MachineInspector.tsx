...
export function MachineInspector() {
  ...
  if (isPallet) {
    ...
    return <aside className="machine-inspector panel"><div className="inspector-top"><div><div className="inspector-kicker">STOK KONUMU</div><h2>{object.name}</h2><span className="machine-state running">● STOKTA</span></div><button onClick={() => select(null)} type="button">×</button></div><div className="inspector-tabs"><b>Stok Detayı</b><span>Kalite</span><span>Hareketler</span></div><dl className="machine-metrics"><div><dt>Malzeme</dt><dd>{material}</dd></div><div><dt>Palet tipi</dt><dd>{isInput ? "Makine girdi paleti" : "Makine çıktı paleti"}</dd></div><div><dt>Mevcut miktar</dt><dd>{quantity} adet</dd></div><div><dt>Kalite kontrolü tamamlanan</dt><dd>{qualityChecked} adet</dd></div><div><dt>Kalite kontrolü bekleyen</dt><dd>{qualityPending} adet</dd></div><div><dt>Reddedilen</dt><dd>0 adet</dd></div><div><dt>Son kalite kontrolü</dt><dd>{measuredAt}</dd></div><div><dt>Ölçülen numune</dt><dd>2 adet</dd></div><div><dt>Ölçüm sonucu</dt><dd>UYGUN</dd></div><div><dt>Kapasite</dt><dd>24 adet</dd></div><div><dt>Doluluk</dt><dd>{Math.round((quantity / 24) * 100)}%</dd></div><div><dt>Son hareket</dt><dd>2 dk önce</dd></div></dl><p className="last-update">Stok bilgisi: Ritim Stok ve Kalite simülasyonu</p></aside>;
  }
